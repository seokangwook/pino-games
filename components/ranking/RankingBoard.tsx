...
type Row = { user_id: string; moves: number; time_ms: number; profiles?: { nickname: string } | null }
...
export function RankingBoard({ gameType, gridSize }: { gameType: 'cards' | 'mahjong'; gridSize?: string }) {
  ...
  return (
    <table className="w-full text-sm">
      ...
      <tbody>{rows.map((r, i) => {
        const nick = (r as any).profiles?.nickname || r.user_id.slice(0,8) + '...'
        return (
          <tr key={i} className={`border-b border-[#FFD4A8]/30 ${i < 3 ? 'font-bold' : ''}`}>
            <td className="py-2">{i===0?'🥇':i===1?'🥈':i===2?'🥉':i+1}</td>
            <td className="py-2 text-[#6B4C2A] truncate max-w-[140px]">{nick}</td>
