...
export function GameResultModal({ result, myMatched, opMatched, totalPairs, elapsed, opElapsed, onRematch, onHome }: GameResultModalProps) {
  const { m } = useT()
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
      <div className={`w-full max-w-sm rounded-3xl shadow-2xl px-8 py-10 text-center flex flex-col items-center gap-4
        ...
        <div className="text-7xl leading-none">{isVictory ? '🎉' : '😿'}</div>

        <h1 className={`text-4xl font-black tracking-tight leading-none ${isVictory ? 'text-yellow-600' : 'text-rose-500'}`}>
          {isVictory ? m.gameResult.victory : m.gameResult.defeat}
        </h1>

        <p className="text-sm text-[#A0785A] font-medium -mt-1">
          {isVictory ? m.gameResult.victoryMsg : m.gameResult.defeatMsg}
        </p>

        <div className="w-full bg-white/70 rounded-2xl px-5 py-4 flex flex-col gap-2 text-sm">
          <div className="flex justify-between">
            <span className="text-[#A0785A] font-semibold">{m.gameResult.myMatched}</span>
            <span className="font-black text-[#6B4C2A]">{myMatched}/{totalPairs}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[#A0785A] font-semibold">{m.gameResult.elapsed}</span>
            <span className="font-black text-[#6B4C2A]">{formatTime(elapsed)}</span>
          </div>
          <hr className="border-[#FFD4A8]/60 my-1" />
          <div className="flex justify-between">
            <span className="text-[#A0785A] font-semibold">{m.gameResult.opMatched}</span>
            <span className="font-black text-[#6B4C2A]">{opMatched}/{totalPairs}</span>
          </div>
          {opElapsed != null && (
            <div className="flex justify-between">
              <span className="text-[#A0785A] font-semibold">{m.gameResult.opClear}</span>
        ...
        <div className="w-full flex flex-col gap-2 mt-1">
          <button onClick={onRematch}
            className={`w-full py-3 rounded-2xl font-black text-white text-base transition-opacity hover:opacity-90 active:scale-95 ${isVictory ? 'bg-yellow-400' : 'bg-[#FFB7C5]'}`}>
            {m.gameResult.rematch}
          </button>
