...
export function NicknameModal() {
  const { needsNickname, role, saveNickname } = useAuthStore()
  const { m } = useT()
  ...
  useEffect(() => {
    ...
    const validErr = nick.trim() ? validateNickname(nick, role) : null
  ...
  async function handleSave() {
    if (!canSave) return
    const finalErr = validateNickname(nick.trim(), role)
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-3xl p-8 shadow-2xl max-w-sm w-full text-center">
        ...
        <h2 className="text-2xl font-black text-[#6B4C2A] mb-1">{m.auth.nicknameTitle}</h2>
        <p className="text-[#A0785A] text-sm mb-6">{m.auth.nicknameSubtitle}<br /><span className="text-xs text-[#C0A88A]">{m.auth.nicknameHint}</span></p>
        <div className="relative mb-2">
          <input type="text" value={nick} onChange={e => setNick(e.target.value.slice(0, 10))}
        ...
        <div className="flex justify-between items-center text-xs mb-3 px-1">
          <span className={`${error ? 'text-red-500' : 'text-[#A0785A]'}`}>{error || (checking ? m.auth.nicknameChecking : '')}</span>
        ...
        <button onClick={handleSave} disabled={!canSave || saving}
          ...
          {saving ? m.auth.nicknameSaving : m.auth.nicknameSave}
        </button>
        <p className="text-xs text-[#C0A88A] mt-3">{m.auth.nicknameSharedHint}</p>
      </div>
    </div>
  )
}
