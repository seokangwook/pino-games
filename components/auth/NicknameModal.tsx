  ...
  useEffect(() => {
    ...
    if (!validErr && nick.trim().length >= 2) {
      ...
      const t = setTimeout(async () => {
        ...
        setDuplicateError((dup?.length ?? 0) > 0 ? '이미 사용 중인 닉네임입니다' : null)
        setChecking(false)
      }, 300)
      setTimer(t)
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-3xl p-8 shadow-2xl max-w-sm w-full text-center">
        ...
        <p className="text-[#A0785A] text-sm mb-6">랭킹에 표시될 이름을 정해주세요<br /><span className="text-xs text-[#C0A88A]">한글·영문·숫자 2~10자</span></p>
