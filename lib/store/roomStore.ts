...
export const useRoomStore = create<RoomState>((set, get) => ({
  ...
  createRoom: async (gameType, gridSize = '4x4', multiMode = 'concurrent') => {
    ...
    const hostCards = createCardDeck(gridSize)
    const initialBoard = { hostCards, guestCards: [], hostProgress: { matched: 0, moves: 0, clearTime: null }, guestProgress: { matched: 0, moves: 0, clearTime: null }, multiMode, gridSize }
  ...
  joinRoom: async (code) => {
    ...
    const guestCards = createCardDeck(gridSize)
    ...
    set({ roomId: room.id, roomCode: code.toUpperCase(), role: 'guest', gameType: 'cards', gridSize, multiMode: (boardState.multiMode as MultiMode) ?? 'concurrent', status: 'playing', boardState: newBoard, error: null })
    get().subscribeRoom()
    return true
  },
