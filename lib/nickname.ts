// 통합 닉네임 검증 모듈 — 전 앱 공통
const BANNED: string[] = [
  '씨발','시발','씨팔','씨바','씨불','쉬발','존나','졸라','개새끼','새끼','개년','개놈',
  '병신','빙신','미친','찐따','지랄','보지','자지','섹스','강간','변태','창녀',
  '꺼져','닥쳐','뒤져','죽어','fuck','shit','bitch','asshole','bastard','cunt','cock',
  'dick','pussy','whore','slut','nigger','nigga',
...
export function validateNickname(nick: string): string | null {
  ...
  for (const b of BANNED) {
    if (lower.includes(b.toLowerCase()) || norm.includes(normalize(b))) return '사용할 수 없는 닉네임입니다'
  }
  return null
}
